...
typedef union {
    ...
    short get_M_EGS() { return raw >> 3 & 0x1fff; }

    // Sets actual gear
    void set_GIC(GEAR value){ raw = (raw & 0xfffff0ffffffffff) | ((uint64_t)value & 0xf) << 40; }
    // Gets actual gear
    GEAR get_GIC() { return (GEAR)(raw >> 20 & 0xf); }

    // Sets target gear
    void set_GZC(GEAR value){ raw = (raw & 0xffff0fffffffffff) | ((uint64_t)value & 0xf) << 44; }
    // Gets target gear
    GEAR get_GZC() { return (GEAR)(raw >> 16 & 0xf); }
    ...
    void set_FPC_AAD(FPC value){ raw = (raw & 0xfffffffffcffffff) | ((uint64_t)value & 0x3) << 24; }
    // Gets drive program for AAD
    FPC get_FPC_AAD() { return (FPC)(raw >> 38 & 0x3); }
    ...
    uint8_t get_MKRIECH() { return raw >> 48 & 0xff; }
    ...
    uint8_t get_ERROR() { return raw >> 59 & 0x1f; }
    ...
    void set_FEHLPRF_ST(ErrorCheck value){ raw = (raw & 0xffffffffffffff3f) | ((uint64_t)value & 0x3) << 6; }
    // Gets Status error check
    ErrorCheck get_FEHLPRF_ST() { return (ErrorCheck)(raw >> 56 & 0x3); }
