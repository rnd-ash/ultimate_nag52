...
typedef union {
    ...
    // Sets actual gear
    void set_GIC(uint8_t value){ raw = (raw & 0xfffff0ffffffffff) | ((uint64_t)value & 0xf) << 40; }
    // Gets actual gear
    uint8_t get_GIC() { return raw >> 40 & 0xf; }

    // Sets target gear
    void set_GZC(uint8_t value){ raw = (raw & 0xffff0fffffffffff) | ((uint64_t)value & 0xf) << 44; }
    // Gets target gear
    uint8_t get_GZC() { return raw >> 44 & 0xf; }
    ...
    void set_FPC_AAD(uint8_t value){ raw = (raw & 0xfffffffffcffffff) | ((uint64_t)value & 0x3) << 24; }
    // Gets drive program for AAD
    uint8_t get_FPC_AAD() { return raw >> 24 & 0x3; }
    ...
    void set_FEHLPRF_ST(uint8_t value){ raw = (raw & 0xffffffffffffff3f) | ((uint64_t)value & 0x3) << 6; }
    // Gets Status error check
    uint8_t get_FEHLPRF_ST() { return raw >> 6 & 0x3; }
