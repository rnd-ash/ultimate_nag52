...
const uint16_t calc_crc16(uint8_t const *buffer, uint8_t len) {
    uint16_t crc = 0;
    while(len--)
...
namespace SCN_CONFIG {
    ScnConfiguration read_scn() {
        ...
        // Calculate SCN checksum (CRC16-ARC)
        uint16_t crc_result = calc_crc16(read_buffer, SCN_BYTE_LENGTH-2);
        uint16_t crc_scn = (uint16_t)read_buffer[38] | read_buffer[39] << 8;
        cfg.cs_ok = crc_result == crc_scn;
